...
public class TestService {
	...
	public String test() {
		return "{\"message\":\"RADI REST -> /RESTApp/rest/demo/test \"}";
	}
	...
	@Consumes(MediaType.APPLICATION_FORM_URLENCODED)
	public Student testformS(@Form Student st) {
		st.setIme(st.getIme() + "1");
		st.setPrezime(st.getPrezime() + "1");
		return st;	}

	/**
	 * Testira MultivaluedMap tip parametra, koji se koristi kod formi. Ova
	 * vrsta parametra zapravo mapira parametre forme na asocijativnu mapu.
	 * 
	 * @param s
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Student testjson(Student st) {
	 ...
	 * zahtev æemo iskoristiti da iz njega izvuèemo sesiju, a nju æemo
	 * iskoristiti da vežemo objekat klase User na sesiju, pod imenom 'user'.
	 * 
	 * @param request
	 ...
	 * @param request
	 *            JSON string koji reprezentuje objekat klase User.
	 * @return Informacija o tome da li je objekat klase User zakaèen na sesiju,
	 *         ili ne.
	 */
	@GET
	@Path("/testlogin")
